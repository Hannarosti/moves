...
Note: there are lots of ways to plot data using `R`. We are going to use leaflet
...
<!-- Get the map that covers the study area. Zoom out a bit from what calc_zoom suggests. For your own data, you may have to change the zoom. -->
...
```{r}
trk <- mk_track(fisher.dat, .x=location_long, .y=location_lat, .t=timestamp, 
                id = local_identifier, crs = CRS("+init=epsg:4326"))
```
...
trk1 <- trk %>% group_by(id) %>% nest() %>% 
  mutate(dir_abs = map(data, direction_abs, 
                       full_circle = TRUE, zero = "N", clockwise = TRUE), 
