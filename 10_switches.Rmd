...
And we can look at the classified movement trajectory.
```{r, fig.width=10, fig.height=8}
psam <- rstan::extract(fit, pars = c("mu", "rho", "scale", "shape", "stateProbs"))
states <- colMeans(psam$stateProbs[,,2])
ggplot(datos, aes(x,y,col=states)) + 
  geom_point(size=0.5) +
  geom_path(size=0.5) +
...
plot(jitter(z[-1]), states[-200]+1, col = gray(0.5,0.5), 
     ...
     cex.lab = 1.2 , bty = "n", las = 1)
```
Let look at the fitted relationship between the covariate and state probability
```{r}
psam <- rstan::extract(fit, pars = c("alpha", "beta"))

plot(-10, xlim = c(-3,3), ylim = c(0, 1), 
      xlab = "food (centered and standardized)", ylab = "p",
      cex.lab = 1.2 , bty = "n", las = 1)

for(i in 1:500){
  curve(plogis(psam$alpha[i] + psam$beta[i] * x), 
        xlim = c(-3,3), add = TRUE, col = gray(0.5, 0.1))
}

curve(plogis(mean(psam$alpha) + mean(psam$beta) * x),
      add = TRUE, lwd = 3)
